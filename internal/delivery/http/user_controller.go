...
func (controller *UserControllerImpl) FindId(ctx *fiber.Ctx) error {
	userId, err := ctx.ParamsInt("userId")
	if err != nil {
		return fiber.ErrBadRequest
	}

	response, err := controller.UserUsecase.FindById(ctx.UserContext(), uint(userId))
	if err != nil {
		log.Println("failed to find by Id user")
		return err
	}
