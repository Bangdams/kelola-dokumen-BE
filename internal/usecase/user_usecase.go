...
type UserUsecase interface {
	...
	FindById(ctx context.Context, userId uint) (*model.UserResponse, error)
...
type UserUsecaseImpl struct {
	...
	RwListRepo       repository.RwListRepository
	DB               *gorm.DB
	Validate         *validator.Validate
}

func NewUserUsecase(userRepo repository.UserRepository, refreshTokenRepo repository.RefreshTokenRepository, rwListRepo repository.RwListRepository, DB *gorm.DB, validate *validator.Validate) UserUsecase {
	return &UserUsecaseImpl{
		UserRepo:         userRepo,
		RefreshTokenRepo: refreshTokenRepo,
		RwListRepo:       rwListRepo,
		DB:               DB,
		Validate:         validate,
	}
}
...
func (userUsecase *UserUsecaseImpl) FindById(ctx context.Context, userId uint) (*model.UserResponse, error) {
	tx := userUsecase.DB.WithContext(ctx).Begin()
	defer tx.Rollback()
	...
	if err := userUsecase.UserRepo.FindById(tx, user); err != nil {
	...
	log.Println("success find by id from usecase user")
...
func (userUsecase *UserUsecaseImpl) Login(ctx context.Context, request *model.LoginRequest, requestRefreshTokenUser string) (*model.LoginResponse, string, error) {
	...
	if err := userUsecase.UserRepo.Login(tx, user, request.Username); err != nil {
...
func (userUsecase *UserUsecaseImpl) Update(ctx context.Context, request *model.UpdateUserRequest) (*model.UserResponse, error) {
	...
	err = userUsecase.RwListRepo.Update(tx, &rwList)
	if err != nil {
		mysqlErr := err.(*mysql.MySQLError)
		log.Println("failed when update repo Rw List : ", err)
...
func (userUsecase *UserUsecaseImpl) Create(ctx context.Context, request *model.UserRequest) (*model.UserResponse, error) {
	...
	if request.Role == "user" {
		if request.NameRw == "" {
			errorResponse.Message = "invalid request parameter"
			errorResponse.Details = []string{"Field name rw failed on required rule"}
		...
		err := userUsecase.RwListRepo.FindByName(tx, rwList)
		if err == nil {
			errorResponse.Message = "Duplicate entry"
			errorResponse.Details = []string{"name rw already exists in the database."}
		...
		user.RwList.NameRw = request.NameRw
	}
